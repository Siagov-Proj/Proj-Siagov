...
export function TransferList({
            ...
            <div className="h-[320px] overflow-y-auto">
                {Object.keys(groups).length === 0 ? (
                    ...
                    Object.entries(groups).map(([group, items]) => (
                        <div key={group}>
                            {group && groupBy && (
                                <div className="sticky top-0 px-3 py-1.5 bg-blue-50 dark:bg-blue-950/30 text-xs font-semibold text-blue-700 dark:text-blue-400 uppercase tracking-wide border-b">
                                    {group}
                                </div>
                            )}
                            {items.map((item) => (
    ...
    return (
        <div className="flex flex-col sm:flex-row gap-2 items-stretch">
            {renderList(availableGroups, selectedLeft, toggleLeft, searchLeft, setSearchLeft, leftTitle, available.length)}
            ...
            {renderList(assignedGroups, selectedRight, toggleRight, searchRight, setSearchRight, rightTitle, assigned.length)}
        </div>
    );
}
